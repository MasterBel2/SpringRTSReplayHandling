...
final class DataParser {
    ...
    func byte(offsetFromCurrentIndexBy offset: Int) -> UInt8 {
        return data[currentIndex + offset]
    }
    func extraOffsetOfFirstZeroByte(afterOffsetFromCurrentIndex offset: Int) -> Int {
        for (offset, byte) in data[(currentIndex + offset)...].enumerated() {
            if byte == 0 {
                return offset
            }
        }
        fatalError("Could not find zero byte.")
    }

    func withUnsafeUInt8Pointer(block: (UnsafePointer<UInt8>) -> Void, maxCapacity: Int) {
        var dataCopy = Data(data)
        withUnsafePointer(to: &dataCopy) { pointer in
            pointer.withMemoryRebound(to: UInt8.self, capacity: 1) { pointer in
                block(pointer)
            }
        }
